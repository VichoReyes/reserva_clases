defmodule ReservaClasesWeb.AdministratorAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ReservaClasesWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_administrator/3" do
    test "stores the administrator token in the session", %{conn: conn, administrator: administrator} do
      ...
      assert Accounts.get_administrator_by_session_token(token)
    end
  ...
  describe "logout_administrator/1" do
    test "erases session and cookies", %{conn: conn, administrator: administrator} do
      administrator_token = Accounts.generate_administrator_session_token(administrator)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Accounts.get_administrator_by_session_token(administrator_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "administrators_sessions:abcdef-token"
      ReservaClasesWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount: ensure_authenticated" do
    ...
    test "redirects to login page if there isn't a valid administrator_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: ReservaClasesWeb.Endpoint,

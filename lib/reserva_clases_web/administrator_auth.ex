defmodule ReservaClasesWeb.AdministratorAuth do
  use ReservaClasesWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_reserva_clases_web_administrator_remember_me"
  ...
  def log_out_administrator(conn) do
    ...
    administrator_token && Accounts.delete_administrator_session_token(administrator_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      ReservaClasesWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_administrator(conn, _opts) do
    ...
    administrator = administrator_token && Accounts.get_administrator_by_session_token(administrator_token)
  ...
  the current_administrator:
      ...
      defmodule ReservaClasesWeb.PageLive do
        use ReservaClasesWeb, :live_view

        on_mount {ReservaClasesWeb.AdministratorAuth, :mount_current_administrator}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ReservaClasesWeb.AdministratorAuth, :ensure_authenticated}] do
  ...
  defp mount_current_administrator(socket, session) do
    Phoenix.Component.assign_new(socket, :current_administrator, fn ->
      if administrator_token = session["administrator_token"] do
        Accounts.get_administrator_by_session_token(administrator_token)
      end
    end)
  end

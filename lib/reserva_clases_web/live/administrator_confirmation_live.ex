defmodule ReservaClasesWeb.AdministratorConfirmationLive do
  ...
  def mount(%{"token" => token}, _session, socket) do
    form = to_form(%{"token" => token}, as: "administrator")
  ...
  def handle_event("confirm_account", %{"administrator" => %{"token" => token}}, socket) do
    case Accounts.confirm_administrator(token) do
      {:ok, _} ->
        {:noreply,
         socket
